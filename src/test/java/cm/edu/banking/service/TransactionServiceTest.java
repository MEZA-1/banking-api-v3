...
class TransactionServiceTest {
    ...
    // =========================================================================
    //  Skill 10 — Retrait (frais 2 %)
    // =========================================================================

    @Nested
    @DisplayName("Skill 10 — Retrait client via agent (frais 2 %)")
    class RetraitTests {
        ...
        @Test
        @DisplayName("✅ Frais 2 % : client débité de montant+frais, agent crédité du montant")
        void retrait_success_frais2Pourcent() {
            // GIVEN
            BigDecimal montant = new BigDecimal("10000");
            BigDecimal fraisAttendus = new BigDecimal("200.00");   // 10000 × 2%
            BigDecimal debitClient = new BigDecimal("10200.00");

            BigDecimal soldeClientAvant = compteClient.getSolde(); // 200000
            BigDecimal soldeAgentAvant = compteAgent.getSolde();   // 500000
            BigDecimal montantActifAvant = banqueA.getMontantActif();

            Operation opSaved = buildOperation(TypeOperation.RETRAIT, montant, fraisAttendus);
            when(operationRepository.save(any())).thenReturn(opSaved);
            when(mapper.toOperationResponse(any()))
                    .thenReturn(buildOperationResponse(TypeOperation.RETRAIT, montant, fraisAttendus));

            // WHEN
            OperationResponse response = transactionService.effectuerRetrait(
                    agent, new RetraitRequest(client.getId(), montant, null));

            // THEN — vérification des mouvements financiers
            assertThat(compteClient.getSolde())
                    .as("Client débité de montant + frais")
                    .isEqualByComparingTo(soldeClientAvant.subtract(debitClient));

            assertThat(compteAgent.getSolde())
                    .as("Agent crédité du montant seul (sans frais)")
                    .isEqualByComparingTo(soldeAgentAvant.add(montant));

            assertThat(banqueA.getMontantActif())
                    .as("Frais versés au montantActif de la banque")
                    .isEqualByComparingTo(montantActifAvant.add(fraisAttendus));

            // Vérification de l'opération persistée
            ArgumentCaptor<Operation> opCaptor = ArgumentCaptor.forClass(Operation.class);
            verify(operationRepository).save(opCaptor.capture());
            assertThat(opCaptor.getValue().getFrais())
                    .isEqualByComparingTo(fraisAttendus);
            assertThat(opCaptor.getValue().getType())
                    .isEqualTo(TypeOperation.RETRAIT);
        }

        @Test
        @DisplayName("✅ Calcul frais : 100 000 FCFA → frais = 2 000 FCFA")
        void retrait_calcul_frais_100000() {
            // GIVEN
            BigDecimal montant = new BigDecimal("100000");
            BigDecimal fraisAttendus = new BigDecimal("2000.00");
            BigDecimal debitTotal = new BigDecimal("102000.00");

            // soldeClient = 200000 >= 102000 ✓
            Operation opSaved = buildOperation(TypeOperation.RETRAIT, montant, fraisAttendus);
            when(operationRepository.save(any())).thenReturn(opSaved);
            when(mapper.toOperationResponse(any()))
                    .thenReturn(buildOperationResponse(TypeOperation.RETRAIT, montant, fraisAttendus));
            ...
            assertThat(captor.getValue().getFrais())
                    .isEqualByComparingTo(fraisAttendus);
            assertThat(banqueA.getMontantActif())
                    .isEqualByComparingTo(new BigDecimal("4000000").add(fraisAttendus));
        }

        @Test
        @DisplayName("❌ Lève BankingException si solde client < montant + frais")
        void retrait_soldeInsuffisant_throwsBankingException() {
            // GIVEN — soldeClient = 200000, demande 200000 → total requis = 204000
            BigDecimal montant = new BigDecimal("200000");

            // WHEN / THEN
            assertThatThrownBy(() -> transactionService.effectuerRetrait(
                    agent, new RetraitRequest(client.getId(), montant, null)))
                    .isInstanceOf(BankingException.class)
        ...
        @Test
        @DisplayName("❌ Bloque si le compte client est suspendu (Skill 15)")
        void retrait_compteClientSuspendu_throwsBankingException() {
            // GIVEN
            compteClient.setStatut(StatutCompte.SUSPENDU);

            // WHEN / THEN
            assertThatThrownBy(() -> transactionService.effectuerRetrait(
                    agent, new RetraitRequest(client.getId(), new BigDecimal("1000"), null)))
                    .isInstanceOf(BankingException.class)
                    .hasMessageContaining("suspendu");
        }

        @Test
        @DisplayName("❌ Bloque si le compte agent est suspendu (Skill 15)")
        void retrait_compteAgentSuspendu_throwsBankingException() {
            // GIVEN
